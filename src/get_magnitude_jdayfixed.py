...
if __name__ == '__main__':
    ...
    for net_sta in netsta_list:
        ...
        # temporary fix for station MBAR changing metada from 20 Hz to 40 Hz in 2019-09-22
        if sta == 'MBAR':
            picklist_day = args.csv[9:19] + 'T' + args.csv[20:30].replace('-', ':')
            # print(picklist_day)
            picklist_day = UTCDateTime(picklist_day)
            if picklist_day - UTCDateTime('2019-09-22T00:00:00') < 0:
                samp_rate = 20 
    ...
    for i, pick_time in enumerate(df_picks['arrivaltime_utc']):
        ...
        for j, chan in enumerate(chan_list):
            ...
            if args.datadirect:
                ...
                jday = f"{plot_start.julday:03d}"
                fname = f"{sta}.{net}.{loc}.{chan}.{plot_start.year}.{jday}"
                st_1c = read(DATA_MSEED / sta / fname, format="MSEED")
